-- Migration: Fix get_public_proposal_by_slug to handle get_guide_rating correctly
-- The function might be failing because get_guide_rating returns a TABLE
...
BEGIN
  ...
  -- Get guide rating (handle TABLE return correctly)
  -- Use different variable names to avoid ambiguity with column names
  SELECT 
    COALESCE(rating_result.average_rating, 0)::numeric,
    COALESCE(rating_result.review_count, 0)::bigint
  INTO avg_rating, review_count
  FROM get_guide_rating(proposal_record.guide_id) AS rating_result
  LIMIT 1;

  -- Set defaults if no rating (shouldn't be needed due to COALESCE, but just in case)
  ...
  IF blocks_source = 'proposal_blocks' THEN
    SELECT COALESCE(jsonb_agg(
      jsonb_build_object(
        ...
        'hide_until_deposit', COALESCE(b.hide_until_deposit, false),
